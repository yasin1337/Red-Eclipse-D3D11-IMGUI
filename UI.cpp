...
void UI::Render()
{
    ...
    if (hTargetWindow == nullptr)
    {
        std::vector<WindowItem> windowList;
        GetAllWindow(&windowList);
        for (const auto& item : windowList)
        {
            if (_stricmp(item.CurrentProcessName, TARGET_PROCESS) == 0)
            {
                SetTargetWindow(item.CurrentWindow);
                break;
            }
        }
    }
...
{
    if (hTargetWindow == nullptr || !IsWindow(hTargetWindow))
        return;

    RECT rcClient{};
    // Get the client area (interior of the window)
    GetClientRect(hTargetWindow, &rcClient);

    // Convert client rect to screen coordinates
    MapWindowPoints(hTargetWindow, nullptr, (LPPOINT)&rcClient, 2);

    const int width = rcClient.right - rcClient.left;
    const int height = rcClient.bottom - rcClient.top;

    // Cache to prevent redundant SetWindowPos calls
    static RECT lastRect = { 0, 0, 0, 0 };
    if (rcClient.left == lastRect.left && rcClient.top == lastRect.top &&
        rcClient.right == lastRect.right && rcClient.bottom == lastRect.bottom)
    {
        return;
    }
    lastRect = rcClient;
